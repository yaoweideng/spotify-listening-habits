...
get_file <- function(x) {
  filepath <- paste0("/Users/Wei/Documents/Coding Projects/Spotify/", x)
...
get_50_avg <- function(x) {
  x <- stats::filter(x, rep(1/50, 50), sides = 1)
  return(x)
}
...
tracks_50_avg <- tracks_met
tracks_met <- data.frame(tracks_met[1:6], apply(tracks_met[7:12], 2, get_avg))

# processing pt. 2 --------------------------------------------------------

tracks_50_avg <- data.frame(tracks_50_avg[1:6], apply(tracks_50_avg[7:12], 2, get_50_avg))
tracks_50_avg <- tracks_50_avg %>%
  drop_na()
...
tracks_50_avg <- gather(tracks_50_avg, key = metric, value = measure, metrics)
...
plot3 <- ggplot(tracks_50_avg, aes(x = date_added, y = measure, group = metric, color = metric)) + 
  geom_line() +
  scale_color_manual(values = colors) +
  ylab('measure') + 
  ggtitle('rolling 50-song average metric values of Spotify library over time') +
  theme(axis.title.x = element_blank())

plot4 <- ggplot(tracks_50_avg, aes(x = date_added, y = measure, group = metric, color = metric)) + 
  geom_line() +
  scale_color_manual(values = colors) +
  facet_wrap(~ metric, scales = "free") +
  theme(axis.title.x = element_blank())
